...
Create resized image variants for every source image under `assets/images` with fixed height `420px`, without modifying original files.

## Scope

- Create a dedicated output directory for resized images.
- Generate one resized image per source image in `assets/images`.
...
- [x] Every image in `assets/images` has a corresponding image in `assets/images/420`.
- [x] Each generated image has height `420px`.
- [x] Original images in `assets/images` remain unchanged.
