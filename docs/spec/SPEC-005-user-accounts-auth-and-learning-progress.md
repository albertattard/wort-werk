...
- PostgreSQL is the only target database for runtime and DB-backed verification in the current design.
...
- [x] PostgreSQL is integrated and migrations are versioned for auth + passkey credential baseline.
- [x] Passwordless authentication (WebAuthn passkeys) is implemented end-to-end.
- [x] No password hash storage is required for primary login flow.
- [x] Auth-only slice explicitly excludes attempts/progress/adaptive-selection UI.
- [x] Deferred progress workstream is tracked by separate tasks.
