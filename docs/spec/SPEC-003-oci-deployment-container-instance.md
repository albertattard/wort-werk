...
- OCI DevOps runtime state repair must remain idempotent when a targeted ingress resource is already managed in remote Terraform state; a repeated rollout must skip or tolerate that import instead of failing the deployment preflight
