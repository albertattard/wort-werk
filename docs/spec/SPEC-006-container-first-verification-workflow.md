...
- OCI DevOps image publication must use buildx commands that are supported by the managed runner's Docker CLI implementation instead of assuming every local subcommand or flag is available.
- OCI DevOps build scripts should detect whether the runner exposes `docker buildx create` and `docker buildx build` publication flags at all, and only use builder bootstrap or inline push flags when the managed runner actually supports them.
...
- [ ] OCI DevOps publish scripts adapt to the managed runner's available `docker buildx` subcommands and publication flags instead of assuming builder-bootstrap or inline push support.
