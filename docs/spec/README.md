# Specifications

Each feature lives in its own `SPEC-XXX-*.md` file and includes status plus acceptance criteria.

## Index

- [Product Vision](./Product-Vision.md)
- [SPEC-001: German Article Quiz MVP](./SPEC-001-german-article-quiz-mvp.md)
- [SPEC-002: Containerized Production Runtime](./SPEC-002-containerized-production-runtime.md)
- [SPEC-003: OCI Deployment via Container Instance](./SPEC-003-oci-deployment-container-instance.md)
- [SPEC-004: Asset Sync Automation](./SPEC-004-asset-sync-automation.md)
- [SPEC-005: User Accounts, Authentication, and Learning Progress](./SPEC-005-user-accounts-auth-and-learning-progress.md)
- [SPEC-006: Container-First Verification Workflow](./SPEC-006-container-first-verification-workflow.md)
