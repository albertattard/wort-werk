...
VERIFY_BUILD_HASH="${VERIFY_BUILD_HASH:-$(git -C "${REPO_ROOT}" rev-parse --short=7 HEAD 2>/dev/null || true)}"
VERIFY_BUILD_HASH="${VERIFY_BUILD_HASH:-dev}"
export VERIFY_BUILD_HASH
...
podman_up() {
  ...
  if ! podman run --replace --detach \
    ...
    -e WORTWERK_BUILD_HASH="${VERIFY_BUILD_HASH}" \
